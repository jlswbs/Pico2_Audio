// Scattering digital Waveguide network synthesis //
...
float lp[BRANCHES] = {0,0,0,0};
float baseLowpass = 0.95f;

float S[BRANCHES][BRANCHES];

float freqDependentAttenuation(float freq, int branch){
    return 0.95f - 0.05f * (freq / 1000.0f) * branch;
}
...
inline float lowpassFilter(float in, int branch){
    lp[branch] = lp[branch] + baseLowpass*(in - lp[branch]);
    return lp[branch];
}

enum ExciteType { NOISE, SINE };

void excite(float freq, ExciteType type = NOISE, int branchMask = 0xF) {
...
void initScattering(float freq){
    for(int i=0;i<BRANCHES;i++){
        for(int j=0;j<BRANCHES;j++){
            if(i==j){
                S[i][j] = -0.5f * freqDependentAttenuation(freq,i);
            } else {
                S[i][j] = 0.5f * freqDependentAttenuation(freq,i);
            }
        }
...
void loop() {
    ...
    for(uint32_t n=0;n<I2S_WORDS;n++){
        ...
        float v[BRANCHES];
        for(int b=0;b<BRANCHES;b++)
            v[b] = readWave(wave[b], fractionalIdx[b]);

        float next[BRANCHES]={0};
        for(int i=0;i<BRANCHES;i++){
            for(int j=0;j<BRANCHES;j++)
                next[i] += S[i][j]*v[j];

            next[i] = lowpassFilter(next[i],i);
        }
