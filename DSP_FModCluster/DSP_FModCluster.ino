...
struct FMVoice {
  ...
  FMVoice() : phase(0.0f), modPhase(0.0f), freq(0.0f), 
              modFreq(0.0f), modIndex(0.0f), active(false) {}
  
  void trigger(float frequency, float ratio, float idxmod) {
  ...
  float process() {
    ...
    float modulation = fast_sin(modPhase) * modIndex;
    float sample = fast_sin(phase + modulation);
