// Modal string model base a MIM resonators with wavetable exciter and sympathetic //
...
struct ModalString {
  ...
  float ks[N_MODES];
  float cs[N_MODES];
  float matrix[N_MODES][N_MODES];
  float m = 1.0f;
  float dt = 1.0f / (float)SAMPLE_RATE;
  float velocity = 1.0f;
  float coupling_amount = 1000.0f;
    ...
    for (int i = 0; i < N_MODES; i++) {
      ...
      for (int j = 0; j < N_MODES; j++) {
        if (i == j) matrix[i][j] = 0;
          ...
          matrix[i][j] = coupling_amount / (abs(i - j) + 1.0f);
        }
      }
    }
  ...
  float process() {
    ...
    float next_x[N_MODES];
    float next_v[N_MODES];

    for (int i = 0; i < N_MODES; i++) {

      float coupling_force = 0.0f;
      for (int j = 0; j < N_MODES; j++) {
        coupling_force += matrix[i][j] * x[j];
      }

      float a = (force + coupling_force - ks[i] * x[i] - cs[i] * v[i]) / m;
      v[i] += a * dt;
      x[i] += v[i] * dt;
    ...
    float out = 0;
    for(int i=0; i<N_MODES; i++) out += x[i];
    return out;
